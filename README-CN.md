...
在 [skills.sh](https://skills.sh/) 中查找目标技能，将安装命令中的 skills 改为 skls：

```bash
npx skls add https://github.com/vercel-labs/skills --skill find-skills
```

安装完成后，技能会被复制到 `~/.config/skls` 目录，随后在任意项目中执行：
...
如果待安装技能的目录名和 `~/.config/skls` 中已有一级目录重名：
...
默认会进入交互式界面，列出 `~/.config/skls` 下全部一级目录（包括手动创建的技能）。
...
如果目标目录下已存在同名技能目录，`skls install` 会直接覆盖，不再额外确认。
...
| 命令                         | 说明                                                           |
| ---------------------------- | -------------------------------------------------------------- |
| `npx skls list`              | 列出 `~/.config/skls` 中的全部技能，支持区分托管技能与手动技能 |
| `npx skls update [names...]` | 不带参数时进入交互模式，带参数时更新一个或多个技能             |
| `npx skls remove [names...]` | 不带参数时进入交互模式，带参数时删除一个或多个技能             |
...
> `skls update` 依赖 GitHub API。为避免匿名请求带来的限流限制（每小时 60 次），建议在环境变量中配置 `GITHUB_TOKEN` 或 `GH_TOKEN` 以提升配额（每小时 5000 次）。
