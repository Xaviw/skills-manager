...
describe('skls-mgr cli integration', () => {
  ...
  beforeEach(async () => {
    ...
    await createSkill(
      join(repoDir, 'skills', 'skill-one'),
      'skill-one',
      'managed one',
    );
    await createSkill(
      join(repoDir, 'skills', 'skill-two'),
      'skill-two',
      'managed two',
    );
  });
  ...
  it('adds, lists, installs, and removes skills', async () => {
    ...
    const lockContent = JSON.parse(
      await readFile(join(baseDir, '.skls-mgr-lock.json'), 'utf-8'),
    );
    ...
    const installResult = runCli(
      ['install', '--all', '--dir', './synced-skills', '--copy'],
      projectDir,
      env,
    );
    expect(installResult.exitCode).toBe(0);
    expect(
      existsSync(join(projectDir, 'synced-skills', 'skill-one', 'SKILL.md')),
    ).toBe(true);
    expect(
      existsSync(join(projectDir, 'synced-skills', 'manual-skill', 'SKILL.md')),
    ).toBe(true);

    const removeResult = runCli(
      ['remove', 'skill-one', 'manual-skill'],
      projectDir,
      env,
    );
    ...
    const updatedLockContent = JSON.parse(
      await readFile(join(baseDir, '.skls-mgr-lock.json'), 'utf-8'),
    );
  ...
  it('fails in non-interactive mode when the target directory name already exists', async () => {
    ...
    expect(result.stdout + result.stderr).toContain(
      t('skillDirectoryConflict', { directoryName: 'skill-one' }, locale),
    );
  });
  ...
  it('returns a non-zero exit code for unknown commands', async () => {
    ...
    expect(result.stdout).toContain(
      t('unknownCommand', { command: 'unknown-command' }, locale),
    );
    expect(result.stdout).toContain(t('runHelpForUsage', {}, locale));
  });

  it('supports help aliases and localized help command output', () => {
    const env = {
      USERPROFILE: homeDir,
      HOME: homeDir,
      LC_ALL: 'en-US',
    ...
    const helpResult = runCli(['help'], projectDir, env);
    const shortHelpResult = runCli(['-h'], projectDir, env);

    expect(helpResult.exitCode).toBe(0);
    expect(helpResult.stdout).toContain(t('helpText', {}, 'en'));
    expect(helpResult.stdout).toContain('help                       Show help');
    expect(shortHelpResult).toEqual(helpResult);
  });

  it('supports version aliases', async () => {
    const env = { USERPROFILE: homeDir, HOME: homeDir };
    const packageVersion = JSON.parse(
      await readFile(join(process.cwd(), 'package.json'), 'utf-8'),
    ).version;

    const versionResult = runCli(['version'], projectDir, env);
    const shortVersionResult = runCli(['-v'], projectDir, env);

    expect(versionResult.exitCode).toBe(0);
    expect(versionResult.stdout.trim()).toBe(packageVersion);
    expect(shortVersionResult.stdout.trim()).toBe(packageVersion);
  });

  it('renders Chinese help text when locale resolves to zh', () => {
    const env = {
      USERPROFILE: homeDir,
      HOME: homeDir,
      LC_ALL: 'zh-CN',
    ...
    const result = runCli(['help'], projectDir, env);

    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain(t('helpText', {}, 'zh'));
    expect(result.stdout).toContain('help                       显示帮助');
  });
});
